...
import com.engine.texture.BasicTexture;
...
public class GLRootView extends GLSurfaceView implements GLSurfaceView.Renderer {
	...
	private static final int VERTEX_BUFFER_SIZE = 4 * 4 * 2;
	...
	boolean mIsQueueActive = true;
	TweenManager tweenManager = new TweenManager();

	private static int mDrawingId = 0x1001;

	public GLRootView(Context context) {
		this(context, null);
		FPS.init();
		Tween.registerAccessor(GLNode.class, new SpriteAccessor());
	 ...
	 * @return
	 */
	public static int allocateDrawingId() {
		mDrawingId++;
		return mDrawingId;
	}
	...
	public static ByteBuffer allocateDirectNativeOrderBuffer(int size) {
	...
	public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
		...
		gl.glVertexPointer(2, GL10.GL_FLOAT, 0, mXyPointer);
		gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0, mUvPointer);
		FPS.init();
	}
	...
	public void onSurfaceChanged(GL10 gl1, int width, int height) {
		Log.v(TAG, "onSurfaceChanged: " + width + "x" + height + ", gl10: "
				+ gl1.toString());
		GL11 gl = (GL11) gl1;
		mGL = gl;
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		if (mContentView != null) {
			mContentView.update(); // 执行更新动作
			mContentView.render(GLRootView.this, (GL11) gl); // 执行绘制
		...
		mAnimations.clear();
		FPS.calculateFPS();
		tweenManager.update(FPS.getDeltaTime());
		// 绘制帧数
		if (ENABLE_FPS_TEST) {
			FPS.drawFps(this);
		}
	 ...
	 * @param color
	 */
	public void drawLine(int x, int y, int endx, int endy, int color) {
		float matrix[] = mMatrixValues;
		mTransformation.getMatrix().getValues(matrix);
		drawLine(x, y, endx, endy, color, 2, mTransformation.getMatrix());
	}
	...
	public void drawLine(int x, int y, int endx, int endy, int color,
		...
		float matrix[] = mMatrixValues;
		mTransformation.getMatrix().getValues(matrix);
		drawLine(x, y, endx, endy, color, lineWidth,
				mTransformation.getMatrix());
	}

	private void drawLine(int x, int y, int endx, int endy, int color,
			int lineWidth, Matrix matrix) {
		GL11 gl = mGL;
		if (mTexture2DEnabled) {
			gl.glDisable(GL10.GL_TEXTURE_2D);
			mTexture2DEnabled = false;
		}
		gl.glColor4f(Color.red(color) / 255.0f, Color.green(color) / 255.0f,
				Color.blue(color) / 255.0f, Color.alpha(color) / 255.0f);
		gl.glLineWidth(lineWidth);
		float points[] = mapPoints(matrix, x, y, endx, endy);
		mXyPointer.asFloatBuffer().put(points, 0, 4).position(0);
		gl.glDrawArrays(GL10.GL_LINES, 0, 2);
	}

	private static void putRectangle(float x, float y, float width,
			float height, float[] buffer, ByteBuffer pointer) {
		buffer[0] = x;
		buffer[1] = y;
		buffer[2] = x + width;
		buffer[3] = y;
	...
	public void drawNinePatch(NinePatchTexture tex, int x, int y, int width,
			int height) {
	 ...
	 * 
	 * @param div
	 *            the stretch parts defined in nine-patch chunk
	 * @param source
	 *            the length of the texture
	 * @param target
	 *            the length on the drawing plan
	 * @param u
	 *            output, the positions of these dividers in the texture
	 *            coordinate
	 * @param x
	 *            output, the corresponding position of these dividers on the
	 *            drawing plan
	...
	private void drawMesh(int x[], int y[], float u[], float v[], int nx, int ny) {
		...
		mGL.glDrawElements(GL10.GL_TRIANGLE_STRIP, idxCount,
				GL10.GL_UNSIGNED_BYTE, mIndexPointer);
	}
	...
	public float[] mapPoints(Matrix matrix, int x1, int y1, int x2, int y2) {
	...
	public void clipRect(int x, int y, int width, int height) {
		float point[] = mapPoints(mTransformation.getMatrix(), x, y + height, x
				+ width, y);
	...
	public void drawColor(int x, int y, int width, int height, int color) {
		...
		gl.glBlendFunc(GL10.GL_ONE, GL10.GL_ONE_MINUS_SRC_ALPHA);// ����ȥ��ڱ�
	}

	public void drawTexture(BasicTexture texture, int x, int y, int width,
			int height) {
